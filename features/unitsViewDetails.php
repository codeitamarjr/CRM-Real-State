        ...
        <div class="card-body">
            <table id="dataTable" class="table dt-responsive nowrap" style="width:100%">
                ...
                <tbody>
                    ...
                    $query = "SELECT * FROM unit INNER JOIN tenant ON unit.idunit = tenant.idunit WHERE unit.idunit = $idunit;";
                    $result = mysqli_query($link, $query);
                    while ($row = mysqli_fetch_array($result)) {
                        echo "<tr>";
                        echo "<td>" . $row['movein'] . "</td>";
                        echo "<td>" . $row['leaseExpire'] . "</td>";
                        echo "<td>" . $row['noticeStart'] . "</td>";
                        echo "<td>" . $row['moveOut'] . "</td>";
                        echo "<td>" . htmlspecialchars(getProfile('profileID',$row['profileID'],'firstName')) . ' ' . htmlspecialchars(getProfile('profileID',$row['profileID'],'lastName')) . "</td>";
                        echo "</tr>";
                    } ?>
                </tbody>
            </table>
